...
published: true
tags: [code, layout, citations]
tools: [remark-cite, typescript]
t: true
---

## Introduction to `remark-cite`


`remark-cite` adds support for inline citations and bibliographies to the `remark` markdown processing framework.  The `remark-cite` package contains several subpackages:
...
* `rehype-cite` **(new!)** Renders HTML for inline citations and the bibliography!
* `remark-extract-bibtex` **(new!)** extracts citation data from `bibtex` code blocks

```md title="article.md"
# Article
According to [@meijer1991:functional], a **banana** is a tropical yellow fruit.
```

```text title="references.bib"
@inproceedings{meijer1991:functional,
...
```typescript title="template.ts" del={"a":1} ins={"b":4} {"c":7-9}
