...
# Student's Logic:
since we are not assigning the return value of the function to
any variable, or consuming its output in any other way (like printing),
then Python should be smart enough _not_ to run the function, therefore
launch_missiles() will never run
my_function()
